...
async function predictWaterNeed(formData) {
  const response = await fetch(`${API_BASE_URL}/predict`, {
    ...
    body: JSON.stringify({
      ...
      sol_chaud_sec: formData.solChaudSec === "true",
      Mulching_Used: formData.mulchingUsed === "true",
    ...
    <div className="screen">
      ...
      <div className="content">
        ...
        <div className="section-card">
          ...
          <ResponsiveContainer width="100%" height={160}>
            <LineChart data={moistureData}>
              ...
              <Tooltip
                contentStyle={{ borderRadius: 8, border: "1px solid #e8f5e9", fontSize: 12 }}
              />
              <Line
        ...
        <div className="section-card predict-card">
          ...
          {prediction && (
            <div className={`prediction-result ${prediction.needs_water ? "result-warning" : "result-ok"}`}>
              {prediction.needs_water ? (
                <>
                  ...
                  <p className="result-amount">{prediction.water_quantity_liters ?? prediction.quantity ?? "—"} L</p>
  ...
  return (
    <div className="screen">
      ...
      <div className="content">
        ...
        {result && (
          <div className={`prediction-result ${result.needs_water ? "result-warning" : "result-ok"}`}>
            <div className="result-header">
              {result.needs_water
                ? <TriangleAlert size={18} className="result-icon-warn" />
                : <Check size={18} className="result-icon-ok" />}
              <span className="result-title">
                {result.needs_water ? "Watering Needed" : "No Watering Needed"}
              </span>
            </div>
            {result.needs_water ? (
              <>
                <p className="result-text">Based on the plant image analysis:</p>
                <p className="result-amount">{result.water_quantity_liters ?? result.quantity ?? "—"} L</p>
                <p className="result-text">of water recommended.</p>
              </>
            ) : (
  ...
  return (
    <div className="screen">
      ...
      <div className="content">
        ...
        <div className="settings-section">
          <div className="settings-section-header">
            <Droplets size={16} className="icon-green" />
            <h3 className="settings-section-title">Irrigation Preferences</h3>
          </div>
          <div className="settings-row">
            <div>
            ...
            <div className={`toggle ${autoIrrigation ? "toggle-on" : ""}`} onClick={() => setAutoIrrigation(!autoIrrigation)}>
        ...
        <div className="settings-section">
          <div className="settings-section-header">
            <BellRing size={16} className="icon-green" />
            <h3 className="settings-section-title">Notifications</h3>
          </div>
          <div className="settings-row">
            <div>
              <p className="settings-label">Push Notifications</p>
              <p className="settings-desc">Receive alerts about your plants</p>
            </div>
            <div className={`toggle ${pushNotifs ? "toggle-on" : ""}`} onClick={() => setPushNotifs(!pushNotifs)}>
              <div className="toggle-knob" />
            </div>
          </div>
          <div className="settings-row">
            <div>
              <p className="settings-label">Email Alerts</p>
              <p className="settings-desc">Get daily summary via email</p>
            </div>
            <div className={`toggle ${emailAlerts ? "toggle-on" : ""}`} onClick={() => setEmailAlerts(!emailAlerts)}>
              <div className="toggle-knob" />
            </div>
          </div>
        </div>
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .garden-grid {
          background: #e8f5e9; border-radius: 12px; position: relative;
        ...
        .viewfinder {
          ...
          border-radius: 16px; height: 220px; position: relative;
        ...
        .scan-line-anim {
          position: absolute; width: 80%; height: 2px; background: rgba(255,255,255,0.8);
          animation: scanMove 1.2s ease-in-out infinite alternate;
          box-shadow: 0 0 10px rgba(255,255,255,0.6);
        }
        @keyframes scanMove { from { top: 20%; } to { top: 80%; } }

        .health-bar-wrap { margin: 10px 0; }
        .health-bar-bg { background: #c8e6c9; border-radius: 20px; height: 8px; margin: 6px 0 4px; }
        .health-bar-fill { background: #2e7d32; border-radius: 20px; height: 8px; transition: width 0.5s; }
        .health-score { font-size: 13px; font-weight: 600; color: #2e7d32; }
        ...
        .suggestion-card {
          ...
          display: flex; gap: 12px; border: 1px solid #c8e6c9;
        }
        ...
        /* Settings */
        .action-card {
          background: #fff; border: 1.5px solid #e8f5e9; border-radius: 14px;
          padding: 16px; display: flex; flex-direction: column; align-items: center;
          gap: 8px; font-size: 13px; font-weight: 500; color: #1a1a1a;
        ...
        /* Toggle */
        .toggle {
          width: 44px; height: 24px; border-radius: 20px; background: #ccc;
